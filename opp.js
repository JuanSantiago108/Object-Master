...
const bListPkmn = pokémon
        .filter( p => p.name[0] === "B" )
        .map((p)=> `Result: ${p.name}`)
console.log(bListPkmn)

const pkmnIds = pokémon
        .filter(p => p.id % 3 === 0)
        .map( p => `Id is: ${p.id}, Pokemon name is: ${p.name}`)
console.log(pkmnIds)

const firepokemon = pokémon.filter(p =>p.types.includes('fire'))
console.log(firepokemon)

const moreTwoType = pokémon.filter(p =>p.types.length>1)
console.log(moreTwoType)

const nameAllpokemon = pokémon.map((p)=>`name: ${p.name}` )
console.log(nameAllpokemon)

const idGreater = pokémon
        .filter(p => p.id>99)
...
const fireAndfly = pokémon
        .filter(p =>p.types[1]==='flying')
        .map((p)=>`types: ${p.types[0]}`)
console.log(fireAndfly)
